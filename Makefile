...
CFLAGS=-Wall -g -std=c99 -D_POSIX_SOURCE -D_BSD_SOURCE -m32
CFLAGS_TYPEPATCH=-Doff64_t=__off64_t
LDFLAGS_TYPEPATCH=-L /usr/local/lib -ldwarf -lelf

TYPEPATCH_SRC=src/katana.c src/dwarftypes.c src/dictionary.c src/hash.c src/target.c src/elfparse.c src/util.c src/types.c src/map.c src/hotpatch.c
